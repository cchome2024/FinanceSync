...
财务人员能够在统一平台中快速录入或导入账户余额、收入、支出和收入预测数据，系统在保存前完成结构化校验与异常提示。
...
1. **Given** 财务人员打开数据录入界面，**When** 输入账户余额并保存，**Then** 系统记录条目并显示“保存成功”且可在数据列表中查看。
2. **Given** 财务人员上传包含收入与支出的 Excel/CSV 文件，**When** 系统完成解析，**Then** 平台显示预览表格并允许用户确认或修订后入库。
...
3. **Given** 用户需要导出报表，**When** 点击导出按钮，**Then** 系统生成包含当前筛选条件的标准财务报表文件。

---

### User Story 3 - AI 辅助解析与查询 (Priority: P3)

财务与业务人员可以通过 AI 聊天界面解析非结构化资料并提出自然语言问题，系统返回结构化结果或数据洞察。
...
- **FR-001**: 系统必须提供账户余额、收入、支出、收入预测四类数据的手工录入表单，并支持草稿保存与提交审核两种状态。
- **FR-002**: 系统必须允许上传 CSV、Excel、PDF、图片等文件，调用 AI 解析后生成结构化预览，用户确认后才写入正式库。
...
- **FR-009**: 前端必须在录入与查看页面提供 AI 聊天窗，用于触发解析与查询，系统需记录对话与结果以便审计。
- **FR-010**: 平台必须提供导出功能，将当前筛选条件下的财务数据生成标准报表（含 CSV 与可读版 PDF）。

### Key Entities *(include if feature involves data)*

- **AccountBalance**: 记录各公司账户在特定时间点的余额与理财金额，包含来源文件及确认状态。
- **RevenueRecord**: 存储按大类/子类分组的收入明细，含金额、月份、来源渠道、置信度。
