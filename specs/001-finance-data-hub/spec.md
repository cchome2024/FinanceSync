...
3. **Given** 用户在收入汇总中开启“包含预测”，**When** 切换预测层级或仅查看确定性/非确定性节点，**Then** 表格会以颜色区分预测类型，并展示来自所有预测记录的汇总金额。
4. **Given** 用户需要导出报表，**When** 点击导出按钮，**Then** 系统生成包含当前筛选条件的标准财务报表文件。（当前迭代暂缓实施导出能力，需求保留）
...
- **FR-007A**: 平台必须支持支出预测数据的导入、全量聚合与月度展示，预测现金流卡片需同时输出收入与支出的月度明细，对应金额应基于全部预测记录（独立于公司筛选）并允许颜色区分预测确定性。
...
- **FR-010**: 平台必须提供导出功能，将当前筛选条件下的财务数据生成标准财务报表（含 CSV 与可读版 PDF）。*当前迭代暂缓实施，待后续 Phase 4.2 处理。*
- **FR-011**: 系统必须维护收入、支出、预期收入共用的多层级分类树，分类节点可在导入时自动创建或映射，业务记录需引用分类 ID 以支持树形汇总。
- **FR-012**: 导入流程必须检测余额、收入、支出及预测数据的重复记录，提供覆盖或保持的选项，并在确认阶段支持批量覆盖。
...
- **ExpenseForecast**: 记录预期支出现金流，包括分类、确定性、付款账户、描述及备注；与 ImportJob/Company 关联，并在仪表板的预测现金流卡片中按月汇总。
